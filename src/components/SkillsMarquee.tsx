...
export default function SkillsMarquee() {
  ...
  const devAiSkills: Skill[] = [
    ...
    },
    {
      ...
      icon: (
        <svg viewBox="0 0 24 24" className="w-7 h-7">
          <path d="M14.314 0L2.3 12l3.7 3.7L21.684.013h-7.37z" fill="#54C5F8" />
          <path d="M14.028 11.072L7.558 17.53h7.682l6.46-6.458z" fill="#29B6F6" />
          <path d="M7.558 17.53L14.028 24l1.212-6.47z" fill="#027DFD" />
          <path d="M15.24 17.53L21.7 24H14.028z" fill="#042B59" />
        </svg>
      )
    },
    {
  ...
  const getIcon = (name: string) => {
    const allSkills = [...dataSkills, ...devAiSkills, ...docSkills];
  ...
  return (
    <section className="snap-align-start h-screen w-full flex flex-col justify-between px-6 sm:px-12 lg:px-24 relative overflow-hidden bg-primary py-8 lg:py-10">
      ...
      {/* Mobile/Tablet view: 3 Horizontal Marquees (hidden on desktop) */}
      <div className="lg:hidden max-w-7xl w-full mx-auto flex flex-col gap-5 relative z-10 py-2">
        {/* Track 1: Dados */}
        <div className="flex flex-col gap-1.5">
          <span className="text-xs sm:text-sm font-bold font-mono tracking-wider text-indigo-400 uppercase px-2">
            📊 Dados & Automação
          </span>
          <div className="relative flex overflow-hidden w-full py-1">
            {/* Left and Right Gradient Fades */}
            <div className="absolute left-0 top-0 bottom-0 w-12 bg-gradient-to-r from-primary via-primary/80 to-transparent z-20 pointer-events-none"></div>
            <div className="absolute right-0 top-0 bottom-0 w-12 bg-gradient-to-l from-primary via-primary/80 to-transparent z-20 pointer-events-none"></div>
            
            <div className="flex animate-marquee-left hover:[animation-play-state:paused] whitespace-nowrap">
              {dataSkills.map((skill, index) => renderCard(skill, index, 'data-mob', 'mr-6'))}
              {dataSkills.map((skill, index) => renderCard(skill, index, 'data-mob-dup', 'mr-6'))}
            </div>
          </div>
        </div>

        {/* Track 2: IA & Desenvolvimento */}
        <div className="flex flex-col gap-1.5">
          <span className="text-xs sm:text-sm font-bold font-mono tracking-wider text-purple-400 uppercase px-2">
            💻 Desenvolvimento & IA
          </span>
          <div className="relative flex overflow-hidden w-full py-1">
            {/* Left and Right Gradient Fades */}
            ...
            <div className="flex animate-marquee-right hover:[animation-play-state:paused] whitespace-nowrap">
              {devAiSkills.map((skill, index) => renderCard(skill, index, 'dev-mob', 'mr-6'))}
              {devAiSkills.map((skill, index) => renderCard(skill, index, 'dev-mob-dup', 'mr-6'))}
            </div>
          </div>
        </div>

        {/* Track 3: Documentação */}
        <div className="flex flex-col gap-1.5">
          <span className="text-xs sm:text-sm font-bold font-mono tracking-wider text-emerald-400 uppercase px-2">
          ...
          <div className="relative flex overflow-hidden w-full py-1">
            ...
            <div className="flex animate-marquee-left hover:[animation-play-state:paused] whitespace-nowrap">
              {docSkills.map((skill, index) => renderCard(skill, index, 'doc-mob', 'mr-6'))}
              {docSkills.map((skill, index) => renderCard(skill, index, 'doc-mob-dup', 'mr-6'))}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="hidden lg:grid max-w-7xl w-full mx-auto grid-cols-[0.58fr_0.42fr] gap-8 items-center z-10 flex-1 my-4 overflow-hidden">
        {/* Left Column: Automation Simulator */}
        <div className="flex flex-col h-full justify-center">
          <AutomationSimulator getIcon={getIcon} />
        </div>

        {/* Right Column: 3 Vertical Marquees */}
        <div className="flex gap-4 lg:gap-6 h-[540px] overflow-hidden relative pr-4 justify-between bg-[#0D0C22]/40 border border-[#272835]/40 rounded-3xl p-5 shadow-inner">
          ...
          {/* Col 1: Dados & Automação */}
          <div className="flex flex-col items-center w-24 h-full">
            <span className="text-[10px] font-bold font-mono tracking-wider text-indigo-400 uppercase text-center pb-2.5 border-b border-[#272835]/60 w-full mb-3 select-none">
              📊 Dados
            </span>
            <div className="w-24 overflow-hidden flex-1 relative">
              <div className="flex flex-col animate-marquee-up hover:[animation-play-state:paused] py-1">
                {dataSkills.map((skill, idx) => renderCard(skill, idx, 'v1-desk', 'mb-4'))}
                {dataSkills.map((skill, idx) => renderCard(skill, idx, 'v1-desk-dup', 'mb-4'))}
              </div>
            </div>
          </div>

          {/* Col 2: IA & Desenvolvimento */}
          <div className="flex flex-col items-center w-24 h-full">
            <span className="text-[10px] font-bold font-mono tracking-wider text-purple-400 uppercase text-center pb-2.5 border-b border-[#272835]/60 w-full mb-3 select-none">
              💻 IA & Dev
            </span>
            <div className="w-24 overflow-hidden flex-1 relative">
              <div className="flex flex-col animate-marquee-down hover:[animation-play-state:paused] py-1">
                {devAiSkills.map((skill, idx) => renderCard(skill, idx, 'v2-desk', 'mb-4'))}
                {devAiSkills.map((skill, idx) => renderCard(skill, idx, 'v2-desk-dup', 'mb-4'))}
              </div>
            </div>
          </div>

          {/* Col 3: Documentação & Organização */}
          <div className="flex flex-col items-center w-24 h-full">
            <span className="text-[10px] font-bold font-mono tracking-wider text-emerald-400 uppercase text-center pb-2.5 border-b border-[#272835]/60 w-full mb-3 select-none">
              📝 Docs
            </span>
            <div className="w-24 overflow-hidden flex-1 relative">
              <div className="flex flex-col animate-marquee-up hover:[animation-play-state:paused] py-1">
                {docSkills.map((skill, idx) => renderCard(skill, idx, 'v3-desk', 'mb-4'))}
                {docSkills.map((skill, idx) => renderCard(skill, idx, 'v3-desk-dup', 'mb-4'))}
              </div>
            </div>
          </div>
        </div>
